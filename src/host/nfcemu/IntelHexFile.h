...
public:
    ...
    std::vector<IHexLine>::iterator Begin() {
        return lines.begin();
    }

    std::vector<IHexLine>::const_iterator Begin() const {
        return lines.cbegin();
    }

    std::vector<IHexLine>::iterator End() {
        return lines.end();
    }

    std::vector<IHexLine>::const_iterator End() const {
        return lines.cend();
    }
    ...
    template<typename T>
    void GetImage(T bi) const {
        std::vector<unsigned char> image;
        size_t maxAddr = 0;
        for(auto l : lines) {
            if(l.address+l.data.size() > maxAddr) {
                maxAddr = l.address+l.data.size();
            }
        }
        image.resize(maxAddr);
        for(auto ii = Begin(); ii != End(); ++ii) {
            std::copy(ii->data.begin(), ii->data.end(), image.begin()+ii->address); 
        }
